...
\begin{tabbing}
\\Sea $e$ la semilla correspondiente a un ejemplo en la clase positiva.
\\Sea $N$ el conjunto de ejemplos negativos de la clase C\\
1. \=$G(e,N)\leftarrow \emptyset$ (complejo vacío cubre todas las observaciones) \\
2. Mientras $G(e,N)$ cubra algún ejemplo de $N$:\\
 \>3. Elige un ejemplo negativo $E_{neg}$ cubierto por $G(e,N)$\\
 \>4. \= Especializa los complejos en $G(e,N)$ con el fin de excluir $E_{neg}$\\
 \>\>4.1 Sea $EX$ el conjunto de selectores que cubren $e$ pero no $E_{neg}$ \\
 \>\>4.2 $G(e,N)=\{x \wedge y \vert x \in G(e,N), y \in EX \}$\\
 \>5. Selecciona el mejor complejo de $G(e,N)$\\
\=8. Regresa $G(e,N)$
\end{tabbing}
...
Como podemos observar en el algoritmo \ref{algo:AQ}, al momento de generar los complejos, el algoritmo AQ sólo considera especializaciones que excluyen 
