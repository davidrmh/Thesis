...
Observemos que con esta forma de separar los datos, un conjunto de prueba se convierte en un nuevo conjunto de entrenamiento, esto nos permite capturar el comportamiento dinámico de los mercados, es decir, su comportamiento no estacionario (las reglas que aprendimos en el pasado, no necesariamente funcionarán en el futuro).
...
Tanto como para el algoritmo AQ como para el algoritmo CN2 se realizaron dos tipos de experimentos. El primero de ellos está basado en un enfoque de \textit{aprender-aplicar-descartar}, en el cual, dado un conjunto de entrenamiento, se induce un conjunto de reglas las cuales se aplican al conjunto de prueba más próximo. Después de esto, las reglas aprendidas se descartan (mecanismo de olvido).
...
Para medir el desempeño de las reglas en los conjuntos de entrenamiento de los cuales se inducen por primera vez, utilizamos la suma de su \textit{soporte} y su \textit{exactitud de Laplace}.
...
Por ejemplo, consideremos un caso en el cual la regla de compra, $R_{C}$, nos indica que en el tiempo $t_1$ se debe de realizar una compra, la cual tendrá un precio de ejecución $P_{C}$. Conforme avanza el tiempo, notamos que la regla de venta, $R_{V}$, nos indica que en el tiempo $t_2 > t_1$, se debe de ejecutar una venta la cual tendrá precio de ejecución $P_{V}$. La recompensa (penalización) que se le asigna a cada regla en el par $\left(R_C, R_V\right)$ está dada por su ganancia (pérdida) porcentual, es decir:

\begin{equation}\label{eqn:recompensa reglas}
Recompensa(R_C, R_V) = \dfrac{P_V}{P_C} - 1
\end{equation}
...
La imagen \ref{imagen:aprendizaje_incremental} muestra un esquema del aprendizaje incremental que se propone en este trabajo.

\begin{figure}[h]
\centering
\scalebox{0.8}{\includegraphics[width=1\linewidth]{imagenes/aprendizaje-incremental.jpg}}
