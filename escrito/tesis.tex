...
En este trabajo se propone una metodología para aprender, de manera automática, un conjunto de reglas de la forma \textit{si...entonces}, las cuales nos permitirán generar estrategias de inversión en el mercado accionario tanto de México como de Estados Unidos.

Desde el punto de vista de la teoría económica, este trabajo investiga la hipótesis del mercado eficiente en su versión débil.

\chapter*{Abstract}
The increase in computing power, the digitalization of financial markets and the opportunity to make big profits, are among the factors that have influenced the research and development of computational algorithms whose objective is the guidance in the investment decision process, namely, these algorithms should be able to determine when is the right moment to buy or sell a financial asset.

In this work, we propose a methodology for automatically learning a set of \textit{If...then} rules that will allow us to generate trading strategies for both Mexican and American stock market.

From the point of view of economic theory, this work investigates the weak version of the efficient market hypothesis.
...
En \cite{Hu2015-XCS}, se utiliza aprendizaje reforzado junto con un algoritmo evolutivo para obtener un conjunto de reglas \textit{si..entonces}.\footnote{Extended Classifier System, XCS} Estas reglas son aplicadas para operar en el mercado de índices accionarios de la bolsa de Shangai, se vence a la estrategia \buyhold aún considerando costos de transacción.

Al analizar el estado del arte, se puede observar que las heurísticas evolutivas juegan un papel esencial para obtener las reglas con las cuales se opera en el mercado. Se observa además, que los atributos utilizados en la mayoría de los trabajos, son un conjunto de indicadores técnicos (ver sección \ref{subseccion:indicadores tecnicos}).

Las tablas \ref{tabla:referencia-mercado-tecnica} y \ref{tabla:referencia-venceBH} muestran un resumen del estado del arte recolectado para este trabajo, como se puede observar existen resultados mixtos respecto a la estrategia \buyhold, que comúnmente es utilizada como estrategia de referencia.
\begin{center}
\begin{table}[htbp]
\centering
\begin{tabular}{ccc}
...
\begin{tabular}{ccc}
\hline
\textbf{Referencia} & \textbf{Vence \buyhold} & \textbf{Considera costos de transacción} \\
\hline
...
\hline
\end{tabular}
\caption{\label{tabla:referencia-venceBH} Comparativo del estado del arte por desempeño respecto a \buyhold}
\end{table}
\end{center}
