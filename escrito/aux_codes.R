...
graficaEtiquetado <- function(datos, boolLegend = TRUE){
  ...
  n_obs <- nrow(datos)
  indices_compra <- which(datos$Clase == 1) + h
  indices_venta <- which(datos$Clase == -1) + h
...
graficaBandas <- function(datos, banda_superior = 0.035, banda_inferior = -0.03, indice = 2){
  ...
  plot(datos$Date, datos$Mid, type = "l", lwd = 2.3, xlab = "Fecha", ylab = "Precio de ejecución", main = "Límites de venta")
  points(x = datos$Date[indices_compra[indice]], y = datos$Mid[indices_compra[indice]], cex = 2, pch = 24, bg = "blue")
  ...
  grid(lwd=1, col = "lightgray", lty = "dotted")
  legend(locator(1), legend = c("Compra"), pch = c(24), bty = "n", cex = 1.5, y.intersp = 0.6, pt.bg=c("blue"))
...
##==========================================================================================================
## GRÁFICA DE BUY AND HOLD
## boolLegend es un boolean. TRUE=>Se dibuja leyenda de forma manual.
## utilizar "../datasets/etiquetado-sin-rep-mid-h-1-90-sp500/8_sp500_2010-07-02_2010-11-08_90.csv"
##==========================================================================================================
graficaBuyHold <- function(datos, boolLegend = TRUE){
  ...
  #Pone la clase en ceros
  datos$Clase <- 0
  datos$Clase[indices_compra] <- 1
  datos$Clase[indices_venta] <- -1

  #gráfica de la estrategia
  # Guardar en JPEG tamaño 800 x 676
  main <- "Estrategia buy and hold"
  
  if(boolLegend){
    plot(datos$Date, datos$Mid, type = "l", lwd = 2.3, xlab = "Fecha", ylab = "Precio de ejecución", main = main)
    points(x = datos$Date[indices_compra], y = datos$Mid[indices_compra], cex = 2, pch = 24, bg = "blue")
    points(x = datos$Date[indices_venta], y = datos$Mid[indices_venta], cex = 2, pch = 25, bg = "red")
    lines(x=c(datos$Date[indices_compra], datos$Date[indices_venta]), y = c(datos$Mid[indices_compra], datos$Mid[indices_venta]), lty = 5, lwd = 2.5, col = "darkgreen")
    grid(lwd=.8, col = "lightgray", lty = "dotted")
    legend(locator(1), legend = c("Compra", "Venta"), pch = c(24, 25), bty = "n", cex = 1.5, y.intersp = 0.6, pt.bg = c("blue", "red"))  
  }
  else{
