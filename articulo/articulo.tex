...
For every test set the excess return over the buy-and-hold strategy and the total monetary gain for every \$100,000.00, are calculated. Tables \ref{table:AQ results} and \ref{table:CN2 results} show the results.

As we can see, AQ algorithm is able to outperform buy-and-hold strategy when using incremental learning.

On the other hand, CN2 algorithm performs poorly in all the scenarios, the reason behind this is because even when CN2 by default gives us an ordered set of rules, we're not preserving such order when rewarding (penalizing) rules performance over the test sets. 
