...
Each individual in the population represents a particular trading strategy for the given training period and is encoded as a vector $\mathbf{x} = \left(x_1, x_2, \ldots, x_t \right)$ whose components, $x_{i}$, represent the action for the day $i$ with $ x_{i} \in \{-1,0,1 \}$, where $-1,0,1$ represent a sell, hold and buy signal respectively. 

We used \textit{excess return} over the buy-and-hold strategy as the fitness function and a population of $60$ individuals with $40$ generations.
...
For both learning algorithms, AQ and CN2, we realized two type of experiments. The first of them is based on a \textit{learn-apply-discard} approach, in which we first induce a set of rules, apply them to the corresponding test set and then discard them for future test sets unless that the rules are induced again in future training sets.
...
The reward(penalization) given to each pair of buy-sell rules, is the percentage gain (loss) they caused, that is:

\begin{equation} \label{eqn:reward}
reward(B,S) = \dfrac{P_{sell}  (1 - c) }{P_{buy}  (1 + c)} -1 
\end{equation}

where $reward(B,S)$ is the reward or loss for a pair of buy-sell rules acting together, $P_{sell}$ and $P_{buy}$, are the sell and buy price respectively and $c$ is the transaction cost per operation.

Finally for each rule that did not appear in the test set being evaluated, we penalize it with the average loss over that same test set, this is as a way of discarding rules due to the pass of time.
