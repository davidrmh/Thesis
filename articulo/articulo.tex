...
\%Gain_{BH} = \dfrac{P_{Sell}^{end}  (1 - c)}{P_{Buy}^{beginning}  (1 + c)} - 1
\end{equation}

where $P_{Sell}^{end}$ is the sell execution price at the end of a prespecified time period, $P_{Buy}^{beginning}$ is the buy execution price at the beginning of the prespecified time period and $c$ is the percentage transaction cost.
